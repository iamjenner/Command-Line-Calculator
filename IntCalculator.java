...
public class IntCalculator {
    ...
    public String infixToPostfix() throws StackException {
        ...
        for(int i = 0; i < infixExpression.length(); i++){
            ...
            if(isDigit(c)){
                // see whether the number has more than a ones place
                StringBuilder number = new StringBuilder();
                number.append(c);
                while(i+1 < infixExpression.length() && isDigit(infixExpression.charAt(i+1))){
                    number.append(infixExpression.charAt(i+1));
                    i++;
                }
        ...
        for(int i = 0; i < postfixExpression.length(); i++){
            ...
            if(isWhiteSpace(c)){
                ...
                while(i+1 < postfixExpression.length() && isDigit(postfixExpression.charAt(i+1))){
                    number.append(postfixExpression.charAt(i+1));
                    i++;
                }
                int numToPush = Integer.parseInt(number.toString());
